...
const getMovieDetail = async id => {
  const result = await fetchData(`movie/${id}`, [language]);
...
const getMovieVideos = async id => {
  let rawResult = await fetchData(`movie/${id}/videos`, [language]);
...
const getMovieSimilars = async id => {
  const rawResult = await fetchData(`movie/${id}/similar`, [language]);
